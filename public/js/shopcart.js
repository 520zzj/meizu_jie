(function(){
                ...
                <ul class="list_item">
                    <li class="col-choose">
                        ...
                        <div class="desc">
                            ...
                            <p class="config">${p.net}&nbsp;${p.color}&nbsp;${p.intsto}</p>
                        </div>
                    </li>
                    <li class="col-price">
                    ...
                    <li class="col-mount">
                        <div class="mount_box">
                            <span class="disable">-</span><input type="text" value=${p.mount} data-toggle="mount"><span>+</span>
                        </div>
                    </li>
                    <li class="col-subtotal"> 
                        <em>￥</em><span>${p.mount*p.unitprice}</span>
                    </li>
                    <li class="col-edit">
    ...
    //获取导航栏右上角订单处的元素
    var orderbox=document.querySelector("[data-nav=ordLogReg]")
    //获取登录提示处元素
    var logTip=document.querySelector("[data-toggle=logTip]")
    //获取sessionstorage判断登录状态，根据不同状态拼接不同HTML代码
    if(sessionStorage.getItem("login")=="true"){
        //如果已经登录了
        orderbox.innerHTML=`<li><a href="" class="olr_link">我的订单</a></li>`
        logTip.style.display="none"
        //如果是登录状态，并且sessionstorage里面存有商品信息
        if(sessionStorage.getItem("cart")){
            ...
            //转换成js对象
           // cart=JSON.parse(cart),不用转js对象，地址栏只能传字符串
            console.log(cart)
               //先发送ajax请求，把sessionstorage里面的商品信息存到数据库
            ajax({
                method:"post",
                url:"http://127.0.0.1:9000/shopcart/addMore",
                dataType:"json",
                data:`cart=${cart}&uid=${uid}`//一次把商品信息发送到后台服务器
            }).then(result=>{
                //成功批量插入数据库后，删除本地sessonstorage里面的商品信息
                if(result.code==1){
            ...
            //如果session里面没有信息不用发送ajax向数据库插入信息，直接刷新商品列表
            ajax({
                method:"get",
                url:"http://127.0.0.1:9000/shopcart",
                dataType:"json"
        ...
        <li><a href="http://127.0.0.1:5500/log_reg.html?action=reg" class="olr_link">注册</a></li>`
        logTip.style.display="flex"
        //如果是未登录状态，并且session里面存有商品信息，直接把sessionstorage商品显示在页面
        if(sessionStorage.getItem("cart")){
            var uid=sessionStorage.getItem("uid")
            //获取session里面的商品信息
            var cart=sessionStorage.getItem("cart")
                    ...
                    <div class="pro_lists">
                        <ul class="list_item">
                            <li class="col-choose">
                                ...
                                <a href="" class="img_link">
                                        <img class="little_img"
                                        src=${res[i].img_src} alt="">
                                </a>
                                <div class="desc">
                                    <h3 class="name">${res[i].pname}</h3>
                                    <p class="config">${res[i].net}&nbsp;${res[i].color}&nbsp;${res[i].intsto}</p>
                                </div>
                            </li>
                            <li class="col-price">
                                <em>￥</em><span data-toggle="unipri">${res[i].unitprice}</span>
                            </li>
                            <li class="col-mount">
                                <div class="mount_box">
                                    <span class="disable">-</span><input type="text" value=${res[i].mount} data-toggle="mount"><span>+</span>
                                </div>
                            </li>
                            <li class="col-subtotal"> 
                                <em>￥</em><span>${res[i].mount*res[i].unitprice}</span>
                            </li>
                            <li class="col-edit">
                                <div data-id=${i}>--</div>
                            </li>
            ...
            //获取删除确认框元素
                        ...
                        li.children[0].addEventListener("click",function(){
                            ...
                            if(li.children[0].innerHTML=="x"){
                                ...
                                var id=document.querySelector("[data-id]")
                                  ...
                                  //确认框隐藏
                                  delTip.style.display="none"
                                  //如果删掉的是最后一个商品，则直接刷新页面，不调用loahtml2()
                                  if(cart.length==0){
                ...
                (function(){
                    ...
                    for(var selBox of selBoxs){
                        //把顺号放到数组里面
                        id.push(selBox.parentNode.parentNode.lastElementChild.children[0].dataset.id)
                    }
                    //将索引倒叙
                    id.reverse();
                    //再一个个删，相当于从后面删数组，删数组时不影响原下标
                   ...
                   //再转回js对象，以便下面使用
                   cart=JSON.parse(cart)
                   //如果所有商品都删除了，不再调用loadhtml2()，直接刷新页面
                   if(cart.length==0){
                        window.history.go(0)
                   }else{
                        //刷新商品列表
